     ...
     /* criarmos uma classe para ter acesso ao BD e criarmos dois métodos  de consulta:
      ...
      class Fornecedor 
      {
        ...
        public static function select(int $id)
        {
            ...
            $tabela = "fornecedor"; //variável para nome da tabela
            $coluna = "idFornecedor"; //variável para chave primaria
        ...
        {
            $tabela = "fornecedor"; //uma variável para nome da tabela "alunos"
        ...
        public static function insert($dados)
        {
            $tabela = "fornecedor"; //uma variável para nome da tabela "alunos"
            $connPdo = new PDO(dbDrive.':host='.dbHost.'; dbname='.dbName, dbUser, dbPass);
            $sql = "insert into $tabela (CNPJ,nomeFornecedor) values (:CNPJ,:nomeFornecedor)";
            $stmt = $connPdo->prepare($sql);
            //Mapear os parâmetros para obter os dados de inclusão
            $stmt->bindValue(':CNPJ', $dados['CNPJ']);
            $stmt->bindValue(':nomeFornecedor', $dados['nomeFornecedor']);
            $stmt->execute() ;
        ...
        {
            $tabela = "fornecedor"; //uma variável para nome da tabela "alunos"
            $coluna = "idFornecedor"; //uma variável para nome "codigo"
            $connPdo = new PDO(dbDrive.':host='.dbHost.'; dbname='.dbName, dbUser, dbPass);
            $sql = "update $tabela set CNPJ=:CNPJ, nomeFornecedor=:nomeFornecedor where $coluna=:idFornecedor";
            $stmt = $connPdo->prepare($sql);
            //Mapear os parâmetros para obter os dados de inclusão
